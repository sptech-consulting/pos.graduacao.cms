...
export const Route = createFileRoute("/aluno_/entrar")({
  beforeLoad: async () => {
    ...
    if (user?.role === "aluno" && user.status === "ativo") throw redirect({ to: "/aluno" });
  },
    ...
    <div className="min-h-screen flex items-center justify-center bg-muted px-4">
      <div className="w-full max-w-md">
        ...
        <div className="rounded-xl border border-border bg-card p-8 shadow-sm">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Use o e-mail cadastrado pelo seu admin.</p>
          ...
          <div className="mt-4 flex items-center justify-end gap-3 text-xs">
            <Link to="/esqueci-senha" search={{ role: "aluno" }} className="text-primary hover:underline">Esqueci minha senha</Link>
          </div>
        </div>
      </div>
    </div>
